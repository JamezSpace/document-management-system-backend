...
class BusinessFunctionEventsAdapter implements BusinessFunctionEventsPort {
    ...
    async businessFunctionCreated(payload: { businessFunctionId: string; }): Promise<void> {
        await this.eventBus.emit({
			eventName: GlobalEventTypes.document.business_function.BUSS_FUNCTION_CREATED,
			occurredAt: new Date(),
			payload,
		});
    }
