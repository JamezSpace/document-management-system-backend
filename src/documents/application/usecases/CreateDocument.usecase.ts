...
import type { MediaServicePort } from "../../../shared/application/port/mediaService.port.js";
...
class DocumentCreation {
    ...
    async saveDocument(payload: Document, file: Buffer) {
        const {mediaId} = await this.mediaService.upload(file, payload.ownerId);
        ...
        const savedVersionedDoc = await this.documentVersionRepo.save(firstVersion);
